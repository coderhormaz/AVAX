...
import { Mail, Lock, User, Wallet, ArrowRight, Eye, EyeOff, LogIn, UserPlus } from 'lucide-react';
import { authService, type AuthUser, type LoginCredentials, type SignupCredentials, type LocalLoginData } from '../services/auth';
...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onAuthSuccess }) => {
  const [mode, setMode] = useState<'login' | 'signup' | 'local'>('login');
  const [loginMethod, setLoginMethod] = useState<'email' | 'local'>('email');
  ...
  const [localForm, setLocalForm] = useState<LocalLoginData>({
    username: '',
    privateKey: ''
  });
  ...
  const handleLocalAuth = async () => {
    ...
    try {
      const result = await authService.loginLocally({
        username: localForm.username,
        privateKey: localForm.privateKey || undefined
      });
      ...
      if (result.success && result.user) {
        ...
        setError(result.error || 'Local authentication failed');
      }
    } catch (err) {
      setError('An unexpected error occurred');
    } finally {
